...
"""
This code is to plot elastic stress tensor, interpartilce distance, and doublet fraction's time series.
"""
...
def plot(plot_or_not):
    ...
    #p3, = par2.plot(simul_time[st : et], np.abs(result[4][0,st : et, 2]-result[4][1,st : et, 2]), "g-", label = "interparticle distance in z")
    #p1, = host.plot(simul_time[st : et], result[6][0,st : et, 0], "r-", label = "particle x position", ls = '--')
    p3, = par2.plot(simul_time[st : et], getStress(phi, Ca, 0, 0, ncycle, angle)[:,3][st : et], "g-", label = r'$\sigma _{elas, yx}$')
    ...
    fig.tight_layout()
    if plot_or_not == 1:
        plt.savefig("./TwoCellSystem_TransionalPart_stress_yx_phi_{}_Ca_{}_angle_{}_r_{}_start_{}_end_{}.png".format(phi, Ca, angle, r, st, et), dpi = 300)
    plt.show()

r = 1.0
ncycle = 2000
phi, Ca, angle, st, et = 4.7, 0.15, 30, 0, 100
#phi, Ca, angle, st, et = 6.0, 0.06, -80, 150, 500
plot(0)
'''

test_set = [[3.8, 0.03, -40, 0, 200],
[4.7, 0.04, -70, 100, 300],
...
r = 1.0
ncycle = 2000
for phi, Ca, angle, st, et in test_set:
    plot(0)
'''