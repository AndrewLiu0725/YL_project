...
def clac_stat(time_window, ratio_k):
    ...
    for j in range(2):
        if (count[j]/time_window > ratio_k):
            stat[0, j, 0] += iv[t]
            stat[1, j, 0] += (iv[t])**2
            stat[2, j, 0] += 1
            stat[0, j, 1] += rv[t]
...
phi_range = np.array([9.9, 9.2, 7.9, 6.9, 6.0, 4.7, 3.8]) # [9.9, 9.2, 7.9, 6.9, 6.0, 4.7, 3.8]
...
for phi_index, phi in enumerate(phi_range):
    ...
    for Ca_index, Ca in enumerate(Ca_range):
        ...
        for i in range(2): # run over state category
            if  stat[2, i, 0] > 0: # this state exists
                data[0, i, 0, phi_index, Ca_index] = stat[0, i, 0]/stat[2, i, 0]
                data[0, i, 1, phi_index, Ca_index] = stat[1, i, 0]/stat[2, i, 0] - (stat[0, i, 0]/stat[2, i, 0])**2
                data[1, i, 0, phi_index, Ca_index] = stat[0, i, 1]/stat[2, i, 1]
                data[1, i, 1, phi_index, Ca_index] = stat[1, i, 1]/stat[2, i, 1] - (stat[0, i, 1]/stat[2, i, 1])**2
...
for i in range(2): # run over iv and rv
    ...
    for j in range(2): # run over doublet and kayaking state
        for phi_index, phi in enumerate(phi_range):
            ...
            if j == 0:
                plt.errorbar(Ca_range[nz_range], data[i, j, 0, phi_index, :][nz_range], 
                yerr = data[i, j, 1, phi_index, :][nz_range], label = 'phi = {}, doublet'.format(phi), color = cmap[phi_index])
            elif j == 1:       
                plt.errorbar(Ca_range[nz_range], data[i, j, 0, phi_index, :][nz_range], 
                yerr = data[i, j, 1, phi_index, :][nz_range], label = 'phi = {}, singlet'.format(phi), ls = '--', color = cmap[phi_index])    
...
for Ca_index, Ca in enumerate(Ca_range):
    ...
    plt.savefig('./Pictures/TwoCellSystem_Viscosity_vs_phi_Ca_{}_fixed_ylim_r_{}_timewindow_{}_k_{}.png'.format(Ca, r, time_window, ratio_k), dpi = 300)
