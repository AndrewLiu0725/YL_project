...
def getRotationTime(input_phi, input_Ca, input_criteria_T, input_criteria_Dms, depend, system):
    ...
    for criteria_Dm in criteria_Dms:
        ...
        for i in range(particle_numbers):
            print("start working on particle {}".format(i))
            plt.figure()
            plt.plot(np.arange(timesteps-period), DF_indivisual[i, :])
            ...
            for slice in split:
                plt.plot(slice[0]*np.ones(100), np.linspace(0, 1, 100), linewidth = 2, color = 'r' if slice[2] else 'b')
                plt.plot(slice[1]*np.ones(100), np.linspace(0, 1, 100), linewidth = 2, color = 'r' if slice[2] else 'b')
                plt.plot(np.arange(slice[0], slice[1]), 0.5*np.ones(slice[1]-slice[0]), linewidth = 2, color = 'r' if slice[2] else 'b')
            ...
            plt.savefig('Check/Particle_{}.png'.format(i), dpi = 200)
            plt.close()
...
Ca = random.choice(list(parameter_set[phi].keys()))
ensemble_id = random.choice(parameter_set[phi][Ca])
print("phi = {}, Ca = {}, ensenble_id = {}".format(phi, Ca, ensemble_id))
getRotationTime(phi, Ca, 1, 1, ensemble_id, 1)
